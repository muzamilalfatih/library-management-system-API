                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (reader.Read())
                            {
                                ...
                                return new Result<FullFineDTO>(true, new { success = new { header = "Success", body = "Fine found successfully." } }, fine);
                            }
                            else
                            {
                                return new Result<FullFineDTO>(false, new { error = new { header = "Not Found", body = "Fine not found.." } }, null, 404);
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        return new Result<FullFineDTO>(false, new { error = new { header = "Server Error", body = "An unexpected error occurred on the server." } }, null, 500);
                    }
                }
            }
        }
        ...
        public static async Task<Result<bool>> PayFineAsync(PayFineDTO payFineDTO)
        {
            ...
            using (SqlCommand command = new SqlCommand("sp_PayFine", connection))
            {
                ...
                try
                {
                    ...
                    if (isSuccess)
                    {
                        return new Result<bool>(true, new { success = new { header = "Success", body = "Fine paid successfully." } }, true);
                    }
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    try
                    {
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            ...
                            return new Result<bool>(true, new { success = new { header = "Success", body = "Check for fine complete." } }, hasUnpaidFines);
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    try
                    {
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            ...
                            return new Result<List<FineViewDTO>>(true, new { success = new { header = "Success", body = "All fines retrieved successfully." } }, fines);
                        }
                    }
                    catch (Exception ex)
                    {
                        return new Result<List<FineViewDTO>>(false, new { error = new { header = "Server Error", body = "An unexpected error occurred on the server." } }, fines, 500);
                    }
                }
            }
        }
